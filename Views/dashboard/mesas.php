    ...
    <div class="content">
        ...
        <div class="toolbar">
            <a href="mesas.php?accion=liberarTodas" class="btn-orange"
               onclick="return modalConfirm(event,this,'🪑','¿Liberar todas las mesas?','Todas las mesas cambiarán a estado Disponible y se registrará la acción.')">
            ...
            <div class="mesa-card <?= $cls ?>" <?= $m['estado']==='Ocupada' && $m['pedidos_activos']>0 ? "onclick=\"verPedidoMesa({$m['id']},{$m['numero']})\" style=\"cursor:pointer\"" : '' ?>>
                ...
                <?php if ($m['estado'] === 'Ocupada'): ?>
                    ...
                    <a href="mesas.php?accion=liberar&id=<?= $m['id'] ?>" class="btn-liberar"
                       onclick="event.stopPropagation();return modalConfirm(event,this,'🪑','¿Liberar mesa <?= $m['numero'] ?>?','La mesa cambiará a Disponible y se registrará la fecha y hora.')">Liberar</a>
                <?php endif; ?>
            </div>
            <?php endforeach; ?>
        </div>
